...
.. tip::
   ...
   can be tested with either :guilabel:`Quadrum` or :guilabel:`Solución Factible` as PAC:
...
To include the current :guilabel:`SAT status` to the PDF, click :guilabel:`Update SAT` in the 
...
.. important::
   ...
   is *not* definitive proof that the CFDI is completely cancelled.
...
::guilabel:`Actions` drop-down menu select :guilabel:`Confirm entries`. Select all posted invoices 
again and go back to the  :icon:`fa-gear` :guilabel:`Actions` drop-down menu to select 
...
.. note::
   ...
   Also global invoices created this way won't have a **PDF** in them as their information is
   already within Odoo and is not to be seen by a customer.
