...
#. Convert the final image into `base64` format is not that easy (as this is not really *humain readable*).
...
Focus on `/html_editor` path which is the "new name" of the old Web Editor used
up to Odoo 18 included and replace the placeholders with real data:
